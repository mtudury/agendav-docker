...
sed -i -e "s/AGENDAV_WORKFLOW_URL/$( echo "${AGENDAV_WORKFLOW_URL}" | sed -e 's/[\/}]/\\&/g')/" ${CONFIG_FILE}
sed -i -e "s/AGENDAV_WORKFLOW_KEY/$( echo "${AGENDAV_WORKFLOW_KEY}" | sed -e 's/[\/}]/\\&/g')/" ${CONFIG_FILE}
